...
from model_new import ResNet, MLP
from dataset.data import get_dataloader
from utils import Graph_Vars, set_optimizer, set_log_path, log, print_args, get_scheduler, get_logits_labels, \
    AverageMeter
...
def main(args):
    ...
    graphs1.w_norm = w_norm
    graphs1.h_norm = h_norm
    ...
    for epoch in range(args.max_epochs):
        ...
        if epoch in exam_epochs:
            ...
            log(
                '>>>>EP{}, train loss:{:.4f}, acc:{:.4f}, NC1:{:.4f}, NC2:{:.4f}, NC3:{:.4f}, w-norm:{:.4f}, h-norm:{:.4f}'.format(
                    epoch, graphs1.loss[-1], graphs1.acc[-1], graphs1.nc1[-1], graphs1.nc2[-1], graphs1.nc3[-1],
                    graphs1.w_mnorm[-1], graphs1.h_mnorm[-1]
                ))
            ...
            wandb.log({
                ...
                'train_nc/wnorm': abs(nc_train['w_mnorm'] - w_norm)/w_norm,
                'train_nc/hnorm': abs(nc_train['h_mnorm'] - h_norm)/h_norm,
            },
                step=epoch + 1)
