...
from utils import compute_ETF, compute_W_H_relation, get_logits_labels_feats, analysis_nc1
...
if __name__ == "__main__":
    ...
    ckpts = [fname for fname in os.listdir(args.save_path) if fname.startswith('ep')]
    ...
    for ckpt_idx in ckpts_indices:
        out_sv = {}
        args.ckpt = os.path.join(args.save_path, 'ep{}.pt'.format(args.ckpt))
        ...
        print('CKPT:{}__Pre, Test_acc:{:.4f}, nll:{:.4f}, ece:{:.4f}, adaece:{:.4f}, cece:{:.4f}'.format(args.ckpt, test_acc, p_nll, p_ece, p_adaece, p_cece))
        ...
        logits, labels, feats = get_logits_labels_feats(test_loader, scaled_model)
        test_acc = (logits.argmax(dim=-1) == labels).sum().item() / len(labels)  # on cuda
        ...
        print('CKPT:{}__Post, Test_acc:{:.4f}, nll:{:.4f}, ece:{:.4f}, adaece:{:.4f}, cece:{:.4f}'.format(args.ckpt, test_acc, nll, ece, adaece, cece))
