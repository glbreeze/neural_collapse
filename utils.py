...
def analysis_nc1(logits, targets, feats, num_classes):
    ...
    for computation in ['Mean', 'Cov']:
        ...
        for c in range(num_classes):
            ...
            idxs_cor = ((targets == c) * (targets == preds)).nonzero(as_tuple=True)[0]
            idxs_inc = ((targets == c) * (targets != preds)).nonzero(as_tuple=True)[0]
...
def analysis_nc1_new(logits, targets, feats, num_classes):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
